...
class Flow(object):
    ...
    def __init__(self, flow_el):
        ...
        rxPackets = int(float(flow_el.get('rxPackets')))
        txPackets = int(float(flow_el.get('txPackets')))
        tx_duration = int(float(flow_el.get('timeLastTxPacket')[:-2]) - float(flow_el.get('timeFirstTxPacket')[:-2]))*1e-9
        rx_duration = int(float(flow_el.get('timeLastRxPacket')[:-2]) - float(flow_el.get('timeFirstRxPacket')[:-2]))*1e-9
